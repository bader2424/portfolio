...
function typeName(text){nameTarget.textContent='';let i=0;const tick=()=>{nameTarget.textContent+=text[i]||'';i++;if(i<=text.length)setTimeout(tick,135)};tick()}
...
const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.intersectionRatio>=revealRatio)entry.target.classList.add('visible');else if(!entry.isIntersecting)entry.target.classList.remove('visible');}),{threshold:[0,revealRatio]});
...
Object.assign(copy.en,{emailMe:"View projects"});
Object.assign(copy.ar,{emailMe:"عرض المشاريع"});
